  ...
  try {
    ...
    const mapResult = await telemetryDb.execute(sql`
      WITH LatestGPS AS (
        ...
        ${auth.role === 'dealer' ? sql`WHERE device_id IN (SELECT vehicle_number FROM device_battery_map WHERE dealer_id = ${auth.dealer_id})` : sql``}
        ORDER BY device_id, time DESC
      ),
      LatestBattery AS (
        SELECT DISTINCT ON (vehiclenos) vehiclenos, soc, soh
      ...
      FROM LatestGPS g
      LEFT JOIN LatestBattery b ON b.vehiclenos = g.device_id
    `);
